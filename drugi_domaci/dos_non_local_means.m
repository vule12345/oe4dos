function output_image=  dos_non_local_means (I, K, S, var, h)
%DOS_NON_LOCAL_MEANS Filter image with non local means algorithms
%   Function uses a non local means algorithm with Euclidan weight
%   funcions.
%
%   B = DOS_NON_LOCAL_MEANS(I, K, S, V, H) filters input image I with non
%   local means algorithm using K x K local neighborhoods for calculating
%   similarity and SxS neighborhoods for searching with variance of var and
%   deegree of filtering of h.
...
% Padd input image
padded_img = padarray(I,[pad, pad],'symmetric', 'both');
...
for global_x = pad+1: pad+M
    ...
    for global_y = pad+1: pad+N
        ...
        numerator = max(distance - 2*var, 0);
